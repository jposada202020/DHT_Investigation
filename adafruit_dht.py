...
class DHTBase:
    ...
    def __init__(self, dht11, pin, trig_wait, use_pulseio):
        ...
        self.checksum_test = 0
        self.data_buffer_test = 0
        self.wiring_test = 0
        self.unplausible_test = 0
    ...
    def measure(self):
        ...
        if self._last_called == 0 or (time.monotonic() - self._last_called) > delay_between_readings:
            ...
            if len(pulses) < 10:
                ...
                self.self.wiring_test = self.wiring_test + 1
                raise RuntimeError("DHT sensor not found, check wiring")

            if len(pulses) < 80:
                # We got *some* data just not 81 bits
                self.data_buffer_test = self.data_buffer_test + 1
            ...
            if chk_sum & 0xFF != buf[4]:
                ...
                self.checksum_test = self.checksum_test + 1
                raise RuntimeError("Checksum did not validate. Try again.")

            if new_humidity < 0 or new_humidity > 100:
                # We received unplausible data
                self.unplausible_test = self.unplausible_test + 1
